...
- **dedup/infrastructure/** — Config, persistence, logging.
- **dedup/application/** — Application facades (`ApplicationRuntime`, scan/review/history services) — UI uses these via controllers.
- **dedup/ui/** — CEREBRO shell: CustomTkinter (`ctk_app`, `ctk_pages/`), store, controllers, projections, theme; shared models under `dedup/models/`.
