...
class ProjectionHub:
    ...
    def _handle_event(self, event: ScanEvent) -> None:
        ...
        handlers = {
            ...
            ScanEventType.SESSION_COMPLETED: self._on_scan_completed,
            ScanEventType.SCAN_COMPLETED: self._on_scan_completed,
            ScanEventType.SESSION_CANCELLED: self._on_scan_cancelled,
            ScanEventType.SCAN_CANCELLED: self._on_scan_cancelled,
            ScanEventType.SESSION_FAILED: self._on_scan_failed,
            ScanEventType.SCAN_ERROR: self._on_scan_failed,
        ...
        if not handler:
            return
        with self._lock:
            handler(event.scan_id, event.payload or {})
    ...
    def _on_phase_started(self, sid: str, payload: dict) -> None:
        ...
        self._append_event_log(f"Phase started: {desc or canon}")
    ...
    def _on_phase_completed(self, _sid: str, payload: dict) -> None:
        ...
        self._dirty["phase"] = True
        self._append_event_log(f"Phase completed: {canon}")
    ...
    def _on_resume_validated(self, sid: str, payload: dict) -> None:
        ...
        self._dirty["session"] = True
        self._append_event_log(f"Resume validated: {outcome} — {reason[:60]}")
    ...
    def _on_resume_rejected(self, sid: str, payload: dict) -> None:
        ...
        self._dirty["compatibility"] = True
        self._dirty["session"] = True
        self._append_event_log(f"Resume rejected: restart required — {reason[:60]}")
    ...
    def _on_scan_completed(self, sid: str, payload: dict) -> None:
        ...
        self._metrics = merge_metrics(
            ...
            files_reused_from_prior_inventory=int(
                benchmark.get(
                    ...
                    self._metrics.files_reused_from_prior_inventory,
                ) or 0
            ),
        ...
        self._append_event_log("Scan completed")
    ...
    def _on_scan_failed(self, sid: str, payload: dict) -> None:
        ...
        self._dirty["session"] = True
        self._dirty["terminal"] = True
        self._append_event_log(f"ERROR: {err[:80]}")
    ...
    def _on_phase_checkpointed(self, _sid: str, payload: dict) -> None:
        ...
        if (files - last_files >= 1000 or (now - last_ts) >= 5.0) and canon in self._phases:
            ...
            self._append_event_log(f"Checkpointed: {canon} ({files:,} files)")
    ...
    def _append_event_log(self, message: str) -> None:
        """Append a timestamped event-log entry and mark it dirty."""
        ts = time.strftime("%H:%M:%S")
        self._events_log.insert(0, f"[{ts}] {message}")
        self._dirty["events_log"] = True
