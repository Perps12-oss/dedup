...
class ProvenanceRibbon(ttk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        for label, var in [
            ...
            if col > 0:
                ttk.Separator(self, orient="vertical").grid(row=0, column=col * 2 - 1, sticky="ns", padx=SPACING["lg"])
            lf = ttk.Frame(self, style="Panel.TFrame")
            lf.grid(row=0, column=col * 2, sticky="w")
            ttk.Label(lf, text=f"{label}: ", style="Panel.Muted.TLabel", font=font_tuple("data_label")).pack(
                side="left"
            )
            ttk.Label(lf, textvariable=var, style="Panel.TLabel", font=font_tuple("data_value")).pack(side="left")
            col += 1

        self.columnconfigure(col * 2 - 1, weight=1)

    def update(self, session_id: str = "", verification: str = "", groups: int = 0, reclaimable_bytes: int = 0):
        if session_id:
