...
class EmptyState(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ttk.Label(inner, text=icon, font=font_tuple("empty_icon"), style="Muted.TLabel").pack(pady=(0, SPACING["md"]))
        ttk.Label(inner, text=heading, font=font_tuple("section_title")).pack()
        if message:
            ttk.Label(
                inner, text=message, style="Muted.TLabel", font=font_tuple("body"), wraplength=320, justify="center"
            ).pack(pady=(SPACING["sm"], 0))
        if action_label and on_action:
            ttk.Button(inner, text=action_label, style="Ghost.TButton", command=on_action).pack(pady=(SPACING["lg"], 0))
