...
from ..theme.theme_registry import THEMES, get_display_names, key_from_display_name
...
class TopBar(tk.Frame):
    ...
    def _build(self):
        ...
        self._title_lbl = tk.Label(left, text="CEREBRO", font=font_tuple("section_title"), pady=0)
        self._title_lbl.pack(side="left", padx=(SPACING["sm"], SPACING["md"]))

        self._subtitle_lbl = tk.Label(left, text="Dedup Engine", font=font_tuple("caption"))
        self._subtitle_lbl.pack(side="left")

        # Session chip
        self._session_chip = tk.Label(
            left, text="No session", font=font_tuple("strip"), padx=SPACING["md"], pady=SPACING["xs"], cursor="arrow"
        )
        ...
        self._theme_combo = ttk.Combobox(
            right, textvariable=self._theme_var, values=display_names, state="readonly", width=14
        )
        ...
        self._density_btn = tk.Label(
            right, text="⊞ Cozy", font=font_tuple("strip"), padx=SPACING["md"], pady=SPACING["xs"], cursor="hand2"
        )
        ...
        self._adv_btn = tk.Label(
            right, text="Advanced", font=font_tuple("strip"), padx=SPACING["md"], pady=SPACING["xs"], cursor="hand2"
        )
        self._adv_btn.pack(side="left", padx=(0, SPACING["sm"]))
        self._adv_btn.bind("<Button-1>", lambda e: self._on_advanced_toggle())

        # Settings
        settings_btn = tk.Label(right, text=IC.SETTINGS, font=font_tuple("body"), padx=SPACING["md"], cursor="hand2")
